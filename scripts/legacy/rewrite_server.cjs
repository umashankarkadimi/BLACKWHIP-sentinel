...
function loadIncidents() {
  try {
    ...
    store.state.activeIncidents = store.incidents.filter((i: Incident) => i.status !== 'RESOLVED' && i.status !== 'CLOSED').length;
  } catch (e) {
...
app.post("/api/chat", requireAuth, requireAnalyst, async (req, res) => {
  ...
  let dbIncident: any;
  try {
      const row = db.prepare('SELECT * FROM incidents WHERE id = ?').get(incident_id);
      if (row) {
